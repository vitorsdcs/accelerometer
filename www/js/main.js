...
AccelerometerApp.prototype = {
	...
	_onAccelerometerSuccess: function(acceleration) {
		var that = this;
		that.spanX.textContent = Math.round(acceleration.x * 100) / 100;
		that.spanY.textContent = Math.round(acceleration.y * 100) / 100;
		that.spanZ.textContent = Math.round(acceleration.z * 100) / 100;              
